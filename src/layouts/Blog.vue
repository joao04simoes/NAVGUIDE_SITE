<template>
  <div class="bg-gray-100 min-h-screen flex flex-col">
        ...
        <div v-for="(group, month) in groupedContent" :key="month" class="mb-12">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div v-for="content in group" :key="content.id" 
              ...
              <!-- Date Display -->
              <div :class="content.type === 'interview' ? 'absolute top-2 right-2 text-sm text-white' : 'absolute top-2 right-2 text-sm text-black'"> {{ content.date }}
              </div>
              ...
              :class="content.type === 'interview' ? 'text-white font-medium hover:underline' : 'text-cyan-700 font-medium hover:underline'"
            >Read more
            </button>
            </div>
          </div>
    ...
    <div v-if="selectedContent" class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4" @click.self="closeContent">
      <transition name="fade">
        <div class="bg-white p-6 max-w-2xl w-full rounded-lg shadow-lg transform transition-all">
          ...
          <button @click="closeContent" class="mt-6 px-6 py-2 bg-cyan-600 text-white rounded-full hover:bg-black hover:text-white transition">Close</button>
        </div>
      </transition>
    </div>
  </div>
